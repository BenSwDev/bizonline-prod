...
?>
				...
				<div class="inputLblWrap sendswrap" style="width:100%;margin:0;text-align:start">
					...
					<div style="float:inline-end;">
						<label class="switch" style="float:inline-end;" for="hidePrices" >
						...
						<span style="width:50px;font-size:12px;text-align:left;float:inline-end;padding:4px">הסתר מחירים</span>
					</div>
    ...
    font-size: 12px;display:inline-block;line-height:1;vertical-align:middle;">(למרות שלא נחתמה)</span></div>
						<label class="switch" style="float:inline-end" for="approvedx">
					...
					<div class="signOpt inOrder">
						...
						<label class="switch" style="float:inline-end">
					...
					$phoneClean = str_replace("-","",$order['customerPhone']);
					 ...
					 /*else if(!$order['approved']){*/
						?>
							...
							<div style="float:inline-end;margin:-5px -5px -5px 0">
							<?if($order['customerPhone']) { ?>
...
?>
					...
					<div class="signOpt inOrder">
						...
						<?if(strtotime(date("Y-m-d h:i:s")) <= strtotime($order['showTimeFrom'][0] == 0 ? $lastTreat : $order['showTimeFrom'])){?>
							<?/*php if($enable_review['sendReviews']) {
							?>
							<label class="switch" style="float:inline-end" onclick="swal.fire({icon: 'error',title: 'שינוי הגדרת שליחת חוות דעת ביום עזיבה מתבצע דרך תפריט הגדרות - חוות דעת'});">
