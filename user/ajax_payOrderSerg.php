...
//            }
        ...
        case 'shareInvoice':
            ...
            if (!$payment)
                throw new Exception('Cannot find payment for booking #' . $orderID);
            if (empty($payment['invoice']) || $payment['invoice'] == '-')
                throw new Exception("No invoice found in payment");
            if (empty($phone) || strlen($phone) < 10)
                throw new Exception("Empty or illegal phone number");

            $payData = json_decode($payment['resultData'], true);
            $invData = $payData['invoiceData'] ?? $payData;

            $docData = ['type' => 'invoice', 'siteID' => $siteID];

            if ($payment['invoice'] == '+' && $payData['invoice']){
                $docData['source']  = 'CardCom';
                $docData['docNum']  = $payData['invoice'];
                $docData['docType'] = $payData['invoiceType'] ?? $invData['invoiceType'] ?? 1;
            }
            elseif (!empty($payment['invoiceURL']) && $payment['invoiceURL'] != '+'){     // Yaad
                $docData['source']  = 'Yaad';
                $docData['invoice'] = $payment['invoiceURL'];
            }

            $docKey = DocURL::create($docData);
            $docURL = WEBSITE . 'doc/' . $docKey;
            $msgText = implode(PHP_EOL, [
                'היי ' . $invData['name'] ?? $payData['name'],
                'מצורפת חשבונית מס עבור הזמנה ' . $order['orderIDBySite'],
                'לחצו לצפייה - ' . $docURL,
                '',
                'שימו לב, הקישור פעיל למשך 7 ימים בלבד!']
            );
            ...
            switch($_POST['via']){
                case 'sms':
                    ...
                    $send = Maskyoo::sms($msgText, $phone, $smsName ?: 'BizOnline');
                    $result['success'] = !!$send;
                    break;
...
<div class="pay_order order" id="pay_orderPop" data-order-id="<?=$orderID?>">
    <div class="sendPop" id="dsPop" style="display:none">
        <a href="" target="_blank" id="dsAnc"></a>
        <div class="container">
...
<?php
                    ...
                    if ($canInvoice && ($pay['invoice'] || !in_array($pay['payType'], ['ccard', 'refund', 'unknown'])) && $pay['invoice'] != '-' && ($pay['payType'] != 'coupon' || $stList[$pay['provider']] > 0)){
                        $click = $pay['invoice'] ? ($pay['invoice'] == '-' ? '' : "docSharePop(" . $orderID . ", " . $pay['lineID'] . ", '" . ($pay['invoice'] == '+' ? 'download_invoice.php?orid=' . $orderID . '&pid=' . $pay['lineID'] : YaadPay::$INVOICE_URL . "?" . $pay['invoice']) . "', 'pay" . $pay['lineID'] . "')") : 'openInvoicePop(this)';
?>
