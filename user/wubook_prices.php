<?php
	...
	$que = "SELECT * FROM wubook_sites_periods WHERE siteID = " . $siteID . " AND (`periodType` > 0 OR `dateTo` >= '" . date('Y-m-d') . "')";
	$periods = udb::key_list($que, 'periodType');

    $que = "SELECT h.*, p.periodID FROM `holidays` AS `h` LEFT JOIN `wubook_sites_periods` AS `p` ON (h.holidayID = p.holidayID AND p.siteID = " . $siteID . " AND p.periodType = 0) WHERE h.active = 1 AND h.dateEnd >= CURDATE() AND p.periodID IS NULL";
    $holidays = udb::key_row($que, 'holidayID');

    if (count($holidays)){
        $cross = udb::single_column("SELECT DISTINCT h.holidayID FROM `holidays` AS `h` INNER JOIN `wubook_sites_periods` AS `p` ON (p.siteID = " . $siteID . " AND p.periodType = 0 AND p.holidayID = 0 AND h.dateStart <= p.dateTo AND h.dateEnd >= p.dateFrom) WHERE h.holidayID IN (" . implode(',', array_keys($holidays)) . ")");
        if (count($cross))
            foreach($cross as $cr)
                unset($holidays[$cr]);
    }
    ...
    $minRoomPeriod = udb::key_row("SELECT `weekday`, `minNights`, `minVoid` FROM `wubook_rooms_min_nights` WHERE `roomID` = 0 AND `periodID` = " . $base1['periodID'], 'weekday');
...
$.extend(PriceCaller.prototype, {
    ...
    call: function(force){
        ...
        return $.post('ajax_wubook.php', $.extend({act:'roomPrices'}, this.context)).then(function(res){
    ...
    get_day: function(day){
        return $.post('ajax_wubook.php', $.extend({act:'roomPricesDay', day:day}, this.context)).then(function(res){
            if (res.status === undefined || parseInt(res.status))
                return swal('שגיאה', 'לא מצליח לעדכן מחירים', 'error');
            return res;
        });
    ...
    save_prices: function(data){
        return $.post('ajax_wubook.php', ($.type(data) == 'string') ? data + '&act=savePrices' : $.extend({act:'savePrices'}, data)).then(function(res){
            if (res.status === undefined || parseInt(res.status))
                return swal('שגיאה', 'לא מצליח לעדכן מחירים', 'error');
            checkSync(res.sync);
            return res;
...
function setMinTriggers(div, caller){
    ...
    $(div).find('select:not([name="sync"])').each(function(){
        ...
        $.post('ajax_wubook.php', {act:'saveMinNights', asite:<?=$siteID?>, periodID:caller.context.periodID, type:ex[0], roomID:ex[1], day:ex[2], value:this.value}).then(function(res){
            if (res.status === undefined || parseInt(res.status))
    ...
    $(div).find('select[name="sync"]').on('change', function(){
        $.post('ajax_wubook.php', {act:'sync', asite:<?=$siteID?>, periodID:caller.context.periodID, sync:this.value}).then(function(res){
            if (res.status === undefined || parseInt(res.status))
                return swal('שגיאה', 'לא מצליח לעדכן תקופה', 'error');
            $('#pricePeriods').find('.item.active').click();
        });
...
$(function(){
    ...
    $('#pricePeriods').children('.item').add('#addPeriodBtn').on('click', function(){
        ...
        if($(this).data('period')) {
			...
			$.post('ajax_wubook.php', {act:'getMin', asite:<?=$siteID?>,  periodID:$(this).data('value')}).then(function(res){
				if (res.status === undefined || parseInt(res.status))
					return swal('שגיאה', 'לא מצליח לעדכן תקופה', 'error');
        ...
        caller.get_day($(this).data('index')).then(function(res){
            $('#dayPricePop').empty().html(res.content).fadeIn()
                ...
                .find('.updateButton').on('click', function(){
                    ...
                    $.post('ajax_wubook.php', prm).then(function(res){
                        if (res.status === undefined || parseInt(res.status))
                            return swal('שגיאה', 'לא מצליח לעדכן מחירים', 'error');
    ...
    $('#minRoom0').on('click', 'input[type="checkbox"]', function(){
        ...
        $.post('ajax_wubook.php', {act:'saveWeekend', asite:<?=$siteID?>, periodID:caller.context.periodID, weekend:we}).then(function(res){
            if (res.status === undefined || parseInt(res.status))
                return swal('שגיאה', 'לא מצליח לעדכן תקופה', 'error');
    ...
    $(document).on('click', '.editPeriodPopup .npgo', function(e){
        $.post('ajax_wubook.php', $(this).parents('form').serialize() + '&act=editPeriod&asite=<?=SITE_ID?>').then(function(res){
            if (res.status === undefined || parseInt(res.status))
    ...
    $(document).on('click', '.editPeriodPopup .removeButton', function(e){
		...
		if(pid) {
        $.post('ajax_wubook.php', $(this).parents('form').serialize() + '&act=removePeriod&asite=<?=SITE_ID?>').then(function(res){
            if (res.status === undefined || parseInt(res.status))
    ...
    $('#newPeriod').find('button.npgo').on('click', function(){
        $.post('ajax_wubook.php', $(this.form).serialize() + '&act=newPeriod&asite=<?=SITE_ID?>').then(function(res){
            if (res.status === undefined || parseInt(res.status))
