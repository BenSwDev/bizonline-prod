...
<?php
    ...
    <h2 class="sectionName" style="text-align: right; margin-top: 50px">ספקי אורחי מלון</h2>
    <div class="topMenu"><button id="addHts" class="add-new">הוסף ספק</button></div><div style="clear: both;"></div>
    <div style="text-align: right; margin-top: 20px">
        <div id="htsList">
            <ul >
<?php
        $list = (new SiteItemList($siteID, 'hotel_guest_supplier'))->get_name_list();
        asort($list, SORT_STRING);

        foreach ($list as $id => $name) {
?>
                <li class="editAble hts" data-id="<?=$id?>">
                    <label style="width:375px"><?=$name?></label>
                </li>
<?php
                }
?>
...
<script>
    $(document).ready(function () {
        ...
        $('#htsList').on('click', '.editAble.hts', function(){
            let id = $(this).data('id');
