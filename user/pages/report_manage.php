...
?>
				<tr class='sticky bottom total' id="totalall">
					<td><span>סה"כ</span> <?=$totalOrders?></td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
					<td><?=number_format($totalTreatments);?></td>
					<td>₪<?=number_format($totalTreatCost);?><div><span>ממוצע</span> ₪<?=$totalTreatments? number_format(($totalTreatCost/$totalTreatments),2) : "";?></div></td>
					...
					<td>
						<div><span>להזמנה</span> ₪<?=$totalTreatments? number_format(($totalPrice/$totalOrders),2) : "";?></div>
						<div><span>לטיפול</span> ₪<?=$totalTreatments? number_format(($totalPrice/$totalTreatments),2) : "";?></div>
					</td>
