<style id="theStyle">
	...
	.changever{display:inline-flex;align-items:center;font-size:14px;font-weight:normal;height:30px;cursor:pointer;margin:0 10px}
	...
	#monthtotals_table:not(.ver2) .ver2{display:none}
	#monthtotals_table.ver2 .ver1{display:none}
	
	th.ttime.ver2{font-size:14px}

	#monthtotals_table:not(.selectToPrint) .checkall, 
	#monthtotals_table:not(.selectToPrint) .checkthis{display:none}
	#selectToPrint:checked + .changever::before{background:#0dabb6}	
	#monthtotals_table.selectToPrint tr.unprint{opacity:0.5}

	@media print{
		#monthtotals_table.selectToPrint tr.unprint{display:none}
		#changever{display:none}
	}
...
<label class="changever" for="changever" onclick="$('#monthtotals_table').toggleClass('ver2')">הפרדה לעמודות</label>

<input type="checkbox" style="display:none" id="selectToPrint">
<label class="changever" for="selectToPrint" onclick="$('#monthtotals_table').toggleClass('selectToPrint')">הדפס לפי מטפל</label>

<table id="monthtotals_table" class="bigDamnTable">
   <thead>
      <tr>
         <th class="checkall"><input type="checkbox" onchange="check_all($(this))"></th>
		 <th>ID</th>
         <th>שם המטפל</th>
		 <?if(strpos($sid,',')){?>
		 <th>שם הספא</th>
	  ...
	  <tr class="mini">
		...
		foreach($treatTimes['weekday'] as $ttime){?>
			...
			<th class="ver2 ttime"><?=$ttime?><span> דק'</span><br style="mso-data-placement:same-cell;" /><span>כמות</span></th>
			<th class="ver2 ttime"><?=$ttime?><span> דק'</span><br style="mso-data-placement:same-cell;" /><span>דקות</span></th>
		<?}?>
		<th class="ver1">סה"כ</th>
		<th class="ttime ver2">סה"כ<br style="mso-data-placement:same-cell;" /><span>דקות</span></th>
		<th class="ttime ver2">עלות<br style="mso-data-placement:same-cell;" /><span>דקה</span></th>
		<th class="ttime ver2">סה"כ<br style="mso-data-placement:same-cell;" /><span>תשלום</span></th>
		<?
...
?>
	  <tr class="<?=$master['salary_type']? "highlight" : ""?> unprint" id="<?=$masterID?>" data-sid="<?=$siteID?>" data-month="<?=$selected?>">
         <td class="checkthis"><input type="checkbox" onchange="if($(this).prop('checked')){$(this).closest('tr').removeClass('unprint')}else{$(this).closest('tr').addClass('unprint')}"></td>
...
?>
      <tr class="unprint" style="position:sticky;bottom:0;z-index:1;background:white">
         <td style="line-height:1"><span>סה"כ מטפלים:</span> <?=$mtc?></td>
		 <?if(strpos($sid,',')){?><td></td><?}?>
         <td></td>
         <td></td>
         <td></td>
    ...
    $('#expExcel').on('click', function(){
		$.when(
			$('table#monthtotals_table tr, table#monthtotals_table td, table#monthtotals_table th').each(function(){
				...
				//debugger;
				if($('#monthtotals_table.selectToPrint').length && $(this).hasClass('unprint')){
					$(this).addClass('noExl');
					$(this).find('td').addClass('noExl');
				}
		})
		).then(function(){				
			//return;
...
function check_all(elm){
	if(elm.prop('checked')){
		$('tbody tr:not(:last-child)').removeClass('unprint');
		$('tbody tr .checkthis input').prop('checked',true)
	}else{
		$('tbody tr:not(:last-child)').addClass('unprint');
		$('tbody tr .checkthis input').prop('checked',false)
	}	
}
