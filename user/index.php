<?php
	...
	// new menu fold
	if(intval($_GET['new_menu']==1)){
		$_SESSION['new_menu']=1;
	}else if(intval($_GET['new_menu']==2)) {
		$_SESSION['new_menu']=0;
	}
...
?>
<!DOCTYPE html>
<html dir="<?= htmlspecialchars($dir) ?>" lang="<?= htmlspecialchars($lang) ?>" data-page="<?=$page?>">
    <head>
	...
	<style>
		/*.issue-reporter-widget{display:none}
		div#imageOverlay{display:none}
		div#commentsModal{display:none}	*/
	</style>
...
<select id="language-picker" onchange="changeLanguage(this.value)" class="language-picker" style="display:none">
    ...
    <option value="fr" <?= ($lang === 'fr') ? 'selected' : '' ?>>Français</option>
            ...
            <div class="user logged_in">
                <div data-dontTranslate="1" class="img"><?if($siteData['hostsPicture']){?><img src="/gallery/<?=$siteData['hostsPicture']?>" alt="" /><?}else{ echo mb_substr($_CURRENT_USER->name, 0, 1);}?></div>
				<div class="alerts <?=$alerts || $alerts_wr?  "" : "none"?>">
					<?//=($_CURRENT_USER->id());?>
					<span>
						<?if($alerts_wr || $_GET['a']){?>
							<a class='alert-circle' href="?page=orders&otype=order&orderStatus=active&sourceID=9999&extras=1" style="left:auto;right:0px;background:#115dd3;z-index:4"><?=$alerts_wr?></a>
...
<link rel="stylesheet" href="<?=$_CURRENT_BASE?>assets/css/select2.min.css?4">
...
<script>
    $(document).ready(function() {
		...
		$(document).on("scroll",function(){
			...
			if( viewportTop2 < $(window).scrollTop())
			{
				$("body").addClass('scrolledown');		
			}else{
				$("body").removeClass('scrolledown');		
			}
			viewportTop2 = $(window).scrollTop();
		});

    });
</script>
<?if($_SESSION["new_menu"]){?>
<style>
@media(max-width:992px){
body .r-side > .user{position:fixed}
body.scrolledown:not(.bodyshowheader) header {top: -70px;}
body.scrolledown:not(.bodyshowheader) .r-side > .user {top: -70px;}
html:not([data-page="home"]) body:not(.bodyshowheader) header {top: -70px;}
html:not([data-page="home"]) body:not(.bodyshowheader) .r-side > .user {top: -70px;}
.showheader {transition:0.2s all;position: fixed;top: 0;width: 50px;height: 34px;background: white;z-index: 40;left: 0;right: 0;color: #0dabb6;margin: auto;border-radius: 0 0 20px 20px;border: 1px solid;border-top: 0;}
body.bodyshowheader .showheader{top:140px}
.showheader::after{content:"";position: absolute;border-left: 2px solid;border-top: 2px solid;width: 10px;height: 10px;transform: rotate(-135deg);left: 0;right: 0;margin: auto;top: 6px;}
body.bodyshowheader .showheader::after{transform: rotate(45deg);top: 12px;}
html:not([data-page="home"]) body #calPlus{padding-top:0}
}
</style>
<?}?>
<div onclick="$('body').addClass('scrolledown'); $('body').toggleClass('bodyshowheader')" class="showheader"></div>
