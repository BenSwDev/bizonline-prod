...
function completeMin($periodID){
    ...
    $base = udb::key_row("SELECT a.* FROM `wubook_rooms_min_nights` AS `a` INNER JOIN `wubook_sites_periods` AS `p` USING(`periodID`) WHERE a.roomID = 0 AND p.periodType = 2 AND p.siteID = " . $siteID, 'weekday');
    $curr = udb::key_row("SELECT a.* FROM `wubook_rooms_min_nights` AS `a` WHERE a.roomID = 0 AND a.periodID = " . $periodID, 'weekday');

    $insert = [];
    for($i = 0; $i <= 6; ++ $i)
        $insert[] = "(0, " . $periodID . ", " . $i . ", " . ($curr[$i]['minNights'] ?? $base[$i]['minNights'] ?? 1) . ", " . ($curr[$i]['minVoid'] ?? $base[$i]['minVoid'] ?? 1) . ")";

    $que = "INSERT INTO `wubook_rooms_min_nights`(`roomID`, `periodID`, `weekday`, `minNights`, `minVoid`) VALUES" . implode(',', $insert) . " ON DUPLICATE KEY UPDATE `weekday` = VALUES(`weekday`)";
    udb::query($que);
}

function holiday2period($siteID, $holidayID, $prices = false){
    $hdata = udb::single_row("SELECT * FROM `holidays` WHERE `holidayID` = " . intval($holidayID));
    if (!$hdata)
        throw new Exception('Cannot find holiday ' . $holidayID);

    if ($exists = udb::single_value("SELECT `periodID` FROM `wubook_sites_periods` WHERE `siteID` = " . intval($siteID) . " AND `periodType` = 0 AND `holidayID` = " . $hdata['holidayID']))
        return $exists;

    if ($exists = udb::single_value("SELECT `periodID` FROM `wubook_sites_periods` WHERE `siteID` = " . intval($siteID) . " AND `periodType` = 0 AND `holidayID` = 0 AND `dateFrom` = '" . $hdata['dateStart'] . "' AND `dateTo` = '" . $hdata['dateEnd'] . "'")){
        udb::update('wubook_sites_periods', ['holidayID' => $hdata['holidayID']], "`periodID` = " . $exists);
        return $exists;
    }

    $hot = udb::single_row("SELECT `periodID`, `weekend`, `breakfast` FROM `wubook_sites_periods` WHERE `periodType` = 2 AND `siteID` = " . $siteID);

    $que = "INSERT INTO `wubook_sites_periods`(`siteID`, `periodType`, `holidayID`, `dateFrom`, `dateTo`, `weekend`, `periodName`, `breakfast`)
                SELECT " . $siteID . ", 0, `holidayID`, `dateStart`, `dateEnd`, '" . udb::escape_string($hot['weekend']) . "', `holidayName`, '" . udb::escape_string($hot['breakfast']) . "' FROM `holidays` WHERE `holidayID` = " . $hdata['holidayID'];
    udb::query($que);

    $periodID = udb::last_insert_id();

    if ($prices){
        $fields = udb::single_column("DESCRIBE `wubook_rooms_prices`", 'Field');
        $fields = '`' . implode('`,`', $fields) . '`';

        $que = "INSERT INTO `wubook_rooms_prices`(" . $fields . ") SELECT " . str_replace('`periodID`', $periodID, $fields) . " FROM `wubook_rooms_prices` WHERE `periodID` = " . $hot['periodID'];
        udb::query($que);
    }
...
*/
        case 'sync':
            ...
            if ($periodID < 0){
                $exists = udb::single_value("SELECT `periodID` FROM `wubook_sites_periods` WHERE `periodType` = 0 AND `siteID` = " . $siteID . " AND `holidayID` = " . abs($periodID));
                if ($exists)
                    $periodID = $exists;
            }

            if ($periodID > 0){
                $pdata = udb::single_row("SELECT `holidayID`, `periodType` FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID . " AND `siteID` = " . $siteID);
                if (!$pdata)
            ...
            switch($sync){
                case 1:
                    if ($periodID > 0){
                        udb::query("DELETE FROM `wubook_rooms_min_nights` WHERE `periodID` = " . $periodID);
                        udb::query("UPDATE `wubook_sites_periods` AS `a` INNER JOIN `wubook_sites_periods` AS `b` ON (a.siteID = b.siteID AND b.periodType = 2) 
                                    SET a.weekend = b.weekend, a.breakfast = b.breakfast, a.sync = 1
                                    WHERE a.periodType = 0 AND a.periodID = " . $periodID);
                        udb::query("DELETE FROM `wubook_rooms_prices` WHERE `periodID` = " . $periodID);
                    }
                    break;
                ...
                case 2:
                    if ($periodID > 0){
                        udb::query("DELETE FROM `wubook_rooms_prices` WHERE `periodID` = " . $periodID);
                        udb::query("UPDATE `wubook_sites_periods` SET `sync` = 0 WHERE `periodID` = " . $periodID);
                    } else {
        ...
        case 'getMin':
            ...
            if ($periodID < 0){
                $pdata = udb::single_row("SELECT `periodID`, `periodType`, `weekend`, `sync` FROM `wubook_sites_periods` WHERE `periodType` = 2 AND `siteID` = " . $siteID);
                $periodID = $pdata['periodID'];
                $pType = -1;
            }
            else {
                $pdata = udb::single_row("SELECT `periodID`, `periodType`, `weekend`, `sync` FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID);
                $pType = $pdata['periodType'];
            }

            $minRoomPeriod = udb::key_row("SELECT `weekday`, `minNights`, `minVoid` FROM `wubook_rooms_min_nights` WHERE `roomID` = 0 AND `periodID` = " . $periodID, 'weekday');
            $pm = count($minRoomPeriod);

            // if custom period
            if ($pdata['periodType'] == 0){
                $hotPeriod = udb::single_row("SELECT `periodID`, `weekend` FROM `wubook_sites_periods` WHERE `periodType` = 2 AND `siteID` = " . $siteID);
                $minRoomHot = udb::key_row("SELECT `weekday`, `minNights`, `minVoid` FROM `wubook_rooms_min_nights` WHERE `roomID` = 0 AND `periodID` = " . $hotPeriod['periodID'], 'weekday');
            ...
            $pc = udb::single_value("SELECT COUNT(*) FROM `wubook_rooms_prices` WHERE `periodID` = " . $periodID);
...
<?php
        ...
        case "roomPrices" :
			...
			$que = "SELECT * FROM wubook_sites_periods WHERE periodID = " . $periodID . "";
            ...
            if ($periodID > 0){
                $limits = udb::single_row("SELECT IF(`periodType` = 2, '9999-01-01', `dateFrom`) AS `from`, IF(`periodType` = 1, '9999-01-01', `dateTo`) AS `till`, `periodType`, `weekend` FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID);

                $pricesData = udb::single_row("SELECT * FROM `wubook_rooms_prices` WHERE `periodID` = " . $periodID . " AND `roomID` = " . $roomID . " AND `day` = -1");

                $minRoomPeriod = udb::key_row("SELECT `roomID`, `weekday`, `minNights`, `minVoid` FROM `wubook_rooms_min_nights` WHERE `roomID` IN (0, " . $roomID . ") AND `periodID` = " . $periodID . " ORDER BY `roomID`", 'weekday');
                ...
                if ($limits['periodType'] == 0){
                    ...
                    $hotPeriod = udb::single_row("SELECT `periodID`, `weekend` FROM `wubook_sites_periods` WHERE `periodType` = 2 AND `siteID` = " . $siteID);
                    $minRoomHot = udb::key_row("SELECT `weekday`, `minNights`, `minVoid` FROM `wubook_rooms_min_nights` WHERE `roomID` IN (0, " . $roomID . ") AND `periodID` = " . $hotPeriod['periodID'] . " ORDER BY `roomID`", 'weekday');
                ...
                $minRoomPeriod = udb::key_row("SELECT `roomID`, `weekday`, `minNights`, `minVoid` FROM `wubook_rooms_min_nights` WHERE `roomID` IN (0, " . $roomID . ") AND `periodID` = " . $hotPeriod['periodID'] . " ORDER BY `roomID`", 'weekday');
            }
            ...
            if ($limits['periodType'] == 0 && !$pricesData){
                $que = "SELECT wubook_rooms_prices.*, wubook_sites_periods.weekend FROM `wubook_rooms_prices` INNER JOIN `wubook_sites_periods` USING(`periodID`) WHERE wubook_rooms_prices.roomID = " . $roomID . " AND wubook_rooms_prices.day = -1 AND wubook_sites_periods.periodType > 0 AND wubook_sites_periods.siteID = " . $siteID . " ORDER BY wubook_sites_periods.periodType DESC";
            ...
            $que = "SELECT tmp.date, tmp.periodID
                    ...
                    FROM (
                        ...
                        FROM `year` INNER JOIN `rooms` INNER JOIN `sites` USING(`siteID`)
                            INNER JOIN `wubook_sites_periods` as `p1` ON (sites.siteID = p1.siteID AND p1.periodType = 1)
                            INNER JOIN `wubook_sites_periods` as `p2` ON (sites.siteID = p2.siteID AND p2.periodType = 2)
                            LEFT JOIN `wubook_rooms_prices` as `rp2` ON (rp2.periodID = p2.periodID AND rooms.roomID = rp2.roomID AND rp2.day = year.weekday)
                            LEFT JOIN `wubook_sites_periods` as `p3` ON (sites.siteID = p3.siteID AND p3.periodType = 0 AND year.date BETWEEN p3.dateFrom AND p3.dateTo)
                            LEFT JOIN `wubook_rooms_prices` as `rp3` ON (rp3.periodID = p3.periodID AND rooms.roomID = rp3.roomID AND rp3.day = year.weekday)
                    ...
                    ) AS `tmp` INNER JOIN `wubook_sites_periods` AS `p` USING(`periodID`)
                        INNER JOIN `wubook_rooms_prices` AS `pr` ON (pr.periodID = tmp.ppID AND pr.day = tmp.weekday AND pr.roomID = " . $roomID . ")
...
<?php
        ...
        case "roomPricesDay":
            ...
            if ($periodID > 0){
                $period = udb::single_row("SELECT `periodType`, `periodName`, IF(FIND_IN_SET('" . $weekday . "', `weekend`), 'end', 'day') AS `end`  FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID);
                ...
                $check  = udb::single_value("SELECT pr.periodID FROM `wubook_rooms_prices` AS `pr` INNER JOIN `wubook_sites_periods` AS `p` USING(`periodID`) WHERE pr.day = -1 AND p.periodType = 2 AND p.siteID = " . $siteID);
            ...
            for($i = 0; $i < count($cond); ++$i){
                $que = "SELECT 
                        ...
                        FROM `wubook_sites_periods` AS `p` INNER JOIN `wubook_rooms_prices` AS `pr` ON (pr.periodID = p.periodID)
...
<?php
        ...
        case 'saveMinNights':
            ...
            if ($periodID < 0){
                ...
                $periodID = udb::single_value("SELECT `periodID` FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID . " AND `siteID` = " . $siteID);
            ...
            $pType = udb::single_value("SELECT `periodType` FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID);
            ...
            udb::insert('wubook_rooms_min_nights', $update, true, false);

            if ($pType == 0){
                udb::update('wubook_sites_periods', ['sync' => 0], '`periodID` = ' . $periodID);
        ...
        case 'saveWeekend':
            ...
            udb::update('wubook_sites_periods', ['weekend' => implode(',', array_unique($weekend)), 'sync' => 0], "`periodID` = " . $periodID);

            if ($pType == 2){
                $exs = udb::single_column("SELECT `periodID` FROM `wubook_sites_periods` WHERE `periodType` = 0 AND `sync` = 1 AND `siteID` = " . $siteID);
                if (count($exs))
                    foreach($exs as $ex)
                        udb::update('wubook_sites_periods', ['weekend' => implode(',', array_unique($weekend))], "`periodID` = " . $ex);
            }
            elseif ($pType == 0){
                completeMin($periodID);
            }
        ...
        case "savePricesDay":
            ...
            if ($data['periodID'] < 0){
                $exists = udb::single_value("SELECT `periodID` FROM `wubook_sites_periods` WHERE `periodType` = 0 AND `siteID` = " . $siteID . " AND `holidayID` = " . abs($data['periodID']));
                if ($exists)
                ...
                $data['periodID'] = udb::single_value("SELECT `periodID` FROM `wubook_sites_periods` WHERE `periodID` = " . $data['periodID'] . " AND `siteID` = " . $siteID);
            ...
            //insert row
            udb::insert('wubook_rooms_prices', $data, true, false);

            $pType = udb::single_value("SELECT `periodType` FROM `wubook_sites_periods` WHERE `periodID` = " . $data['periodID']);

            if ($pType == 0){
                udb::update('wubook_sites_periods', ['sync' => 0], '`periodID` = ' . $data['periodID']);
        ...
        case 'newPeriod':
            ...
            $exists = udb::single_row("SELECT `periodID`, `dateFrom`, `dateTo` FROM `wubook_sites_periods` WHERE `siteID` = " . $siteID . " AND `periodType` = 0 AND `dateFrom` <= '" . $till . "' AND `dateTo` >= '" . $from . "'");
            if ($exists)
            ...
            $hot = udb::single_row("SELECT `weekend`, `breakfast` FROM `wubook_sites_periods` WHERE `periodType` = 2 AND `siteID` = " . $siteID);

            udb::insert('wubook_sites_periods', [
                'siteID'     => $siteID,
                'periodType' => 0,
                'dateFrom'   => $from,
                'dateTo'     => $till,
        ...
        case 'editPeriod':
			...
			$holidayID = udb::single_value("SELECT `holidayID` FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID . " AND `siteID` = " . $siteID);
			...
			$exists = udb::single_row("SELECT `periodID`, `dateFrom`, `dateTo` FROM `wubook_sites_periods` WHERE `siteID` = " . $siteID . " AND `periodType` = 0 AND `dateFrom` <= '" . $till . "' AND `dateTo` >= '" . $from . "' AND periodID!=".$periodID);
            if ($exists)
                throw new Exception('Already exists period: ' . db2date($exists['dateFrom']) . ' - ' . db2date($exists['dateTo']));
			...
			udb::update('wubook_sites_periods', ['dateFrom' => $from, 'dateTo' => $till, 'periodName' => $pname], '`periodID` = ' . $periodID);
			...
			case 'removePeriod':
				...
				udb::query("DELETE FROM `wubook_rooms_min_nights` WHERE `periodID` = " . $periodID);
				udb::query("DELETE FROM `wubook_rooms_prices` WHERE `periodID` = " . $periodID);
				udb::query("DELETE FROM `wubook_sites_periods` WHERE `periodID` = " . $periodID);
				$result['status'] = 0;
			break;
