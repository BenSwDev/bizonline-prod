...
?>
<script src="<?=$root;?>/app/AComplete.js?v=<?=time()?>"></script>
<script src="<?=$root;?>/app/WebManager.js?v=1"></script>
<div class="manageItems" id="manageItems">
    <h1>ניהול תפריטים</h1>
	<div class="miniTabs general">
		<?php foreach($languages as $lang){ ?>
			<div class="tab<?=$lang['LangID']==$langID?" active":""?>" onclick="window.location.href='/cms/menus/menuAutoComp/index.php?langID=<?=$lang['LangID']?>&menuType=<?=$menuType?>'"><p><?=$lang['LangName']?></p></div>
		<?php } ?>
	</div>
	<? /*
	<div class="miniTabs general">	
		...
		<form id="manageMenus">
			...
			<div class="rightSide" id="rightSide">
				...
				<div id="freeBox" class="searchWrap">
					<input type="hidden" value="" id="freeSearchParam" name="freeSearchParam" />
					<input type="hidden" value="" id="freeSearchType" name="freeSearchType" />
					<input type="hidden" value="" id="menuTitle" name="menuTitle" />
					<div class="wrapToIco">
						<i class="icon-category"></i>
						<input type="text" placeholder="" name="free" autocomplete="off" id="freeInput" value="">
					</div>
					<div class="autoBox">
...
<script>
 ...
 saveAll();
	...
	$.ajax({
		url: '//'+window.location.hostname+'/cms/menus/menuAutoComp/js_save.php',
		type: 'POST',
		data: formData,
		async: false,
		cache: false,
...
$('.dd').on('change', function() {
   saveAll();
	...
	$.ajax({
		url: '//'+window.location.hostname+'/cms/menus/menuAutoComp/js_order.php?menuType=<?=$menuType?>',
		type: 'POST',
	...
	function removeMenus(menuID){
		$.ajax({
			url: '//'+window.location.hostname+'/cms/menus/menuAutoComp/js_delete.php',
			type: 'POST',
			data: {menuID:menuID},
			async: false,
			success: function (myData) {
	...
	function editMenu(menuID){
		var meID = menuID;
		$.ajax({
			url: '//'+window.location.hostname+'/cms/menus/menuAutoComp/editmenu.php',
			type: 'POST',
...
<style>
    ...
    .rightSide > div:not(.searchWrap) {overflow: hidden;}
	...
	.mainCtrl{    width: calc(100% - 310px);display: inline-block;vertical-align: top;overflow:hidden;clear:both}
