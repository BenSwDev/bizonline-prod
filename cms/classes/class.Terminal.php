...
class Terminal {
    ...
    public static function hasTerminal($siteID, $target = ''){
        ...
        if (!isset(self::$_term_cache[$key])){
            if ($target)
                $data = udb::single_row("SELECT `masof_type`, `active` AS `masof_active`, `masof_number`, `masof_key`, `masof_pwd`, `flag_tokens` AS `masof_no_cvv`, (`flag_cc_charge` + 1) % 2 AS `masof_no_charge` FROM `sites_terminals` WHERE `siteID` = " . $siteID . " AND `target` = '" . udb::escape_string($target) . "'");
            else
                $data = udb::single_row("SELECT `masof_type`, `masof_active`, `masof_number`, `masof_key`, `masof_pwd`, `masof_no_cvv`, `masof_no_charge` FROM `sites` WHERE `siteID` = " . $siteID);
