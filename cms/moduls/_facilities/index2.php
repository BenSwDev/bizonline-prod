...
*/
$attributes_categories = 'attributes_categories_temp';
$attributes_domains = 'attributes_domains_temp';

$attributes = udb::full_list("SELECT * FROM `attributes` WHERE 1 ORDER BY defaultName ASC");
...
?>
                        <li class="dd-item dd3-item attr" data-id="<?=$attr['attrID']?>" data-type="child">
                            ...
                            <div class="attr-type-select" data-test="<?=$attr['attrType']?>">
                                <div class="a-type"><input type="checkbox" title="" data-attr-id="<?=$attr['attrID']?>" value="1" <?=($attr['attrType'] & 1 ? 'checked' : '')?> /></div>
                                <div class="a-type"><input type="checkbox" title="" data-attr-id="<?=$attr['attrID']?>" value="2" <?=($attr['attrType'] & 2 ? 'checked' : '')?> /></div>
                                <div class="a-type"><input type="checkbox" title="" data-attr-id="<?=$attr['attrID']?>" value="4" <?=($attr['attrType'] & 4 ? 'checked' : '')?> /></div>
                            </div>
                        </li>
<?php
            }
?>
				...
				<div class="attr-by-type titles">
					<div class="type-col">אירוח לפי יום</div>
					<div class="type-col">אירוח לפי שעה</div>
					<div class="type-col">אירוח לאירועים</div>
				</div>
			</div>
		</form>
	</div>
...
function openPop(pageID){
	$(".pagePopCont").html('<iframe id="frame_'+pageID+'" frameborder=0 src="/cms/moduls/facilities/frameBase.php?pageID='+pageID+'"></iframe><div class="tabCloser" onclick="closeTab(\'frame_'+pageID+'\')">x</div>');
	$(".pagePop").show();
}
