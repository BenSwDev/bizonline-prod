...
class TempActivePage {
    ...
    const DEFAULT_DOMAIN_ID = 6;
    const DEFAULT_DOMAIN  = 'vii.co.il';
    const USE_IMAGES_DOMAIN = "https://www.vii.co.il";
    const META_TITLE_EXT = 'Vii';
    const ATTRIBUTE_TYPE = 7;
    const ADDTOURL = "";
    const SEARCH_LEVEL2 = 's';
    const BASE_TITLES_START = 'נופש';
...
//        }
    ...
    static public function initSearch($from = 'get'){
        ...
        if (isset($src['pax']) && is_array($src['pax'])){
            ...
            if (count(self::$search['pax']) > 7)
                self::$search['pax'] = array_slice(self::$search['pax'], 0, 7);
        ...
        if ($src['from'] && $src['till']){
            ActivePage::checkDateSearch($src['from'], $src['till']);
    ...
    static public function findSearchAlias(){
        ...
        $sql = "select * from search where  `data` = '" . udb::escape_string($tempSearch) . "' LIMIT 1";
        $searchRecord = udb::single_row($sql);
        if($searchRecord) {
            udb::query("update search set updateDate=NOW() where id=".$searchRecord['id']);
            if ($searchRecord['id'] != self::$page['ref']){
                ...
                $plink = self::showAlias("search",$searchRecord['id'],1,ActivePage::$domainID);
                if ($plink != '' && $searchRecord['active'] == 1) {
                    return $plink . ($pdiff ? '?' . InSearch::stringifyParams($pdiff) : '');
                }
                $que = [];
                $_serahcTitle = searchPageName(self::$search , true);
                $que['title'] = $_serahcTitle;
                udb::update("search_langs",$que," langID=".self::$langID." and id=".$searchRecord['id']);
                $que['count'] = intval($searchRecord['count'] + 1) ;
                udb::update("search",$que," id=".$searchRecord['id']);
            ...
            if(strlen($tempSearch) > 7) {
                ...
                $searchTitleBase = date('Y-m-d H:i:s');

                $query['title'] =  $_serahcTitle;
    ...
    static public function testForRedirect($link){
        if($_SERVER['REMOTE_ADDR'] == "80.179.230.102"){
            $redSQL = "select * from redirects where domainID=". self::$domainID . " and old_url='" . $link  . "'";
            $red = udb::single_row($redSQL);
            if($red) {
                header("Location: " . $red['new_url']);
                exit;
            }
        }
    }
    ...
    static public function getAlias(...$level){
        ...
        $sql = "SELECT * FROM `alias_text` WHERE (`domainID` = " . self::$domainID . " OR `domainID` = 0) AND " . implode(' AND ', $cond) . " ORDER BY `domainID` DESC LIMIT 1";
        //echo $sql;
...
//            self::$cache['roomTypes'] = udb::key_row("SELECT `roomType`, `roomTypeMany`,id FROM `roomTypesLangs` WHERE `langID` = " . self::$langID, 'id');
...
//    }
    ...
    static public function roomTypeName($type, $plural = false){
        ...
        return self::$cache['roomTypes'][$type][$plural!==false ? 'roomTypeMany' : 'roomType'] . ' ' . (( self::$domainID == 1 || self::$domainID == 6) ? 'נופש' : Dictionary::translate('לפי שעה')) ;
    }
        ...
        if(is_array($id)) {
            foreach ($id as $item) {
                if (!isset(self::$cache['attributes'][$item])){
                    ...
                    $returnValue .= $att['connectionValue'];
                }
                else {
                    $att = self::attribute($item);
                    $returnValue .=  $att['connectionValue'];
