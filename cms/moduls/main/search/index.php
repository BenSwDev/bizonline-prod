...
$where =" `search`.domainID=" . $domainID . " ";
if($_GET['free']){
	$where .= "AND (`search`.`title` LIKE '%".udb::escape_string($_GET['free'])."%' OR `search`.`id`='".udb::escape_string($_GET['free'])."' OR alias_text.h1 like '%".udb::escape_string($_GET['free'])."%')";
}
if($_GET['area']){
...
$que="SELECT SQL_CALC_FOUND_ROWS DISTINCT `search`.id ,search_langs.barTitle ,`search`.`domainID`, `search`.`active`, `search`.`count`, `search`.`title` ,alias_text.h1 FROM `search`  
      left join alias_text on (alias_text.ref = search.id and alias_text.`table` = 'search' AND alias_text.LEVEL1 = 'he'  AND alias_text.domainID=".$domainID."  )
      left join search_langs on (`search`.id = search_langs.id and search_langs.langID=1)  
      WHERE " . $where . "   AND `search`.domainID = " . $domainID ." ORDER BY " . $order . $pager->sqlLimit();

$search= udb::full_list($que);
$pager->items_total = udb::single_value("SELECT FOUND_ROWS()");
if(isset($_GET['pg']) && intval($_GET['pg'])) {
    $pager->current_page = intval($_GET['pg']);
}
...
<div class="manageItems" id="manageItems">
    ...
    <?=$pager->render()?>
    מציג <span><?=count($search)?></span> מתוך <span><?=$pager->items_total?></span> <b style="color:red">באדום - קיים לדף מאפיין שאינו מופיע בדומיין או נמחק מרשימת המאפיינים</b>
    <table>
        <thead>
        <tr>
        ...
        <tbody id="sortRow">
            ...
            if(($domainID == 1 || $domainID == 6) && !$page['h1']) {
                ...
                $title = $page['title'];
                //$h1 = udb::single_value("select h1 from alias_text where langID=1 and `table`='search' and ref=".$page['id']);
                $h1 =  $page['h1'];
                if($h1) $title = $h1;
            }
            ...
            <tr id="<?=$page['id']?>" style="<?=$noMatch? "background:#ffaaaa" : "" ?>" data-data='<?=$page['data']?>'>
                ...
                <td onclick="openPop(<?=$page['id']?>, <?=$domainID?>)"><?=$title?></td>
