...
if(intval($_GET['delPage'])!=''){
	...
	udb::query("DELETE FROM `category_articles` WHERE id=".$mainpageid."");
	udb::query("DELETE FROM `category_articles_langs` WHERE id=".$mainpageid."");
	udb::query("DELETE FROM `alias_text` WHERE ref=".$mainpageid." AND `table`='category_articles'");
}


$que="SELECT * FROM `category_articles` WHERE 1";
...
<div class="manageItems" id="manageItems">
    ...
    <table>
        ...
        <tbody id="sortRow">
            ...
            <tr id="<?=$page['id']?>">
                <td><?=$page['id']?></td>
                <td onclick="openPop(<?=$page['id']?>)"><?=outDb($page['title'])?></td>
				<td><?=($page['status']?"<span style='color:green;'>כן</span>":"<span style='color:red;'>לא</span>")?></td>
