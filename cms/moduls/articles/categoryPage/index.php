...
$que="SELECT * FROM `MainPages` WHERE MainPageType=2 ORDER BY `showOrder`";
    ...
    <table>
        ...
        <tbody id="sortRow">
            ...
            <tr id="<?=$page['mainPageID']?>">
				...
				<?php if($pageType!=1) { ?>
				<div onClick="if(confirm('אתה בטוח רוצה למחוק את הדף?')){ location.href='?delPage=<?=$page['mainPageID']?>';  }" class="delete"><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;מחק</div>
				<?php } ?>
				</td>
            </tr>
			<?php }
