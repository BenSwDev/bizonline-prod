...
?>
		<div class="language" data-id="<?=$id?>">
			<div class="inputLblWrap">
				...
				<input type="text" name="siteName" placeholder="<?=js_safe($btr[$id]['siteName'] ?? $siteData['siteName'] ?? 'שם המתחם')?>" value="<?=js_safe($trans[$id]['siteName'])?>" />
			</div>
			<!-- div class="inputLblWrap">
			...
			<div class="inputLblWrap">
				...
				<input type="text" name="owners" placeholder="<?=js_safe($btr[$id]['owners'] ?? $siteData['owners'] ?? 'שם בעלים')?>"  value="<?=js_safe($trans[$id]['owners'])?>" />
			</div>
			<div class="inputLblWrap">
				<div class="labelTo">שם העסק</div>
				<input type="text" placeholder="<?=js_safe($btr[$id]['bussinessName'] ?? $siteData['bussinessName'] ?? 'שם העסק')?>" name="bussinessName" value="<?=js_safe($trans[$id]['bussinessName'])?>" />
			</div>
...
?>
        ...
        <div class="inputLblWrap">
            <div class="labelTo">מזהה פורטלים</div>
            <select placeholder='מזהה פורטלים' name="portalsID" >
                <option value="0">מזהה פורטלים</option>
                <?
                foreach ($villaSites as $villa) {
                    $selected = "";
                    if($siteData['portalsID'] == $villa['siteid']) $selected = " selected ";
        ...
        <div class="inputLblWrap">
            <div class="labelTo">מזהה ספאפלוס</div>
            <input type="text" placeholder='מזהה ספאפלוס' name="spaplusID" value="<?=js_safe($siteData['spaplusID'])?>" />
        </div>
        <div class="inputLblWrap">
            <div class="labelTo">מזהה צימר דקה 90</div>
            <select placeholder='מזהה צימר דקה 90' name="zimmersID" >
                <option value="0">מזהה צימרים</option>
                <?
                foreach ($zimmerSites as $zimmer) {
                    $selected = "";
                    if($siteData['zimmersID'] == $zimmer['siteid']) $selected = " selected ";
 ...
 foreach([$domainID => ''] as $id => $dom){ ?>
       ...
       <?php foreach(LangList::get() as $lid => $lang){
		...
		$trans[$lid] = ($domainID == 1 && $lid == 1) ? $siteData : Translation::sites($siteID, '*', $lid, $domainID);
										...
										<div class="rowWrap">
											...
											<div class="tblCell"><span onclick="galleryOpen(<?=$domid.",".$siteID.",".$gallery['galleryID']?>,'site_main_galleries')"  class="editGalBtn">ערוך גלריה</span>
												<div class="dupGalWrap">
													<select name="galWrapSelect" id="galWrapSelectsite_main_galleries">
...
<?php */ ?>
				...
				<div class="manageItems">
					...
					<?php foreach(DomainList::get() as $domid => $dom){ ?>
						<div class="domain" data-id="<?=$domid?>">
							...
							<div class="addNewBtnWrap">
								<input type="button" class="addNew" id="addNewAcc<?=$domainID?>" value="הוסף חדש" onclick="galleryOpen(<?=$domainID.",".$siteID?>,'new')" >
							</div>
						</div>
					<?php } ?>
			...
			<div class="mainSectionWrapper attr">
                ...
                <select id="locationsTypes" onchange="filterProperties('locationsTypes')">
...
function dupGal(dupsel,galleryID,siteID,curDomain,galleryType){
	if(confirm("האם אתה בטוח שברצונך לשכפל את הגלריה?")){
		var domain = $('#galWrapSelect' + dupsel).val();
