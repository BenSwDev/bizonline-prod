...
if ('POST' == $_SERVER['REQUEST_METHOD']){
    ...
    try {
        ...
        $siteData = [
            ...
            'maxGuestsEvents'=> $data['maxGuestsEvents'],
        ...
        if($domainID == 1)
            $siteData['recommend'] = $data['recommend'][$domainID] ?? 0;

        if (!$roomID){      //  && $domainID==1 opening new room
        ...
        $roomsDomains['roomID'] = $roomID;
        $roomsDomains['domainID'] = $domainID;
        $roomsDomains['active'] = $data['active'][$domainID] ?? 0;
        $roomsDomains['recommend'] = $data['recommend'][$domainID] ?? 0;
        //if(intval($data['roomTypeDomain'])>0){
			$roomsDomains['roomType'] = $data['roomTypeDomain'];
		//}


		udb::insert('rooms_domains',$roomsDomains , true);
        //    }
        ...
        foreach(LangList::get() as $lid => $lang){
            ...
            udb::insert('rooms_langs', [
                ...
                'roomShort' => $data['roomShort'][$domainID][$lid],
...
$all_attributes = udb::full_list("SELECT * FROM `attributes_domains` WHERE active=1 AND domainID > 1 ORDER BY domainID,categoryID, showOrder");
...
if ($roomID){
    ...
    $spaces = udb::full_list("SELECT spaceID,spaceName,spaceType,showOrder FROM `spaces` WHERE roomID=".$roomID. " order by showOrder ASC");
            ...
            <form action="" method="post">
                ...
                <div class="domain" data-id="<?=$domainID?>">
                    <div class="inputLblWrap">
                        ...
                        <label class="switch">
                            <input type="checkbox" name="active" value="1"  <?=($roomDomains[$domainID]['active'] ? 'checked="checked"' : '')?> <?=(!$roomID ? 'checked="checked"' : '')?> onchange="$('#recommend').prop('checked', this.checked)" />
...
?>
				<div class="inputLblWrap">
					<div class="switchTtl">חדר בפירסום</div>
					<label class="switch">
					    <input type="checkbox" name="recommend" id="recommend" value="1" <?=(($roomData['recommend'] || !$roomID) ? "checked" : "")?> />
					    <span class="slider round"></span>
					</label>
				</div>
<?php
...
?>
				...
				<?if($domainID>1){?>
				<div class="inputLblWrap" >
                    <div class="labelTo">סוג היחידה בדומיין</div>
                    <select name="roomTypeDomain">
                        <option value="0">ללא שינוי</option>
						<?php foreach($roomTypes as $type) { ?>
                            <option value="<?=$type['id']?>" <?=($roomDomains[$domainID]['roomType']==$type['id']?"selected":"")?> ><?=$type['roomType']?></option>
                        <?php } ?>
                    </select>
                </div>
                    ...
                    <div class="domain" data-id="<?=$domainID?>">
                        <div class="language" data-id="<?=$lid?>">
                            <div class="section txtarea big">
                                <div class="label">תאור קצר</div>
                                <textarea name="roomShort" class="textEditor"><?=$roomLangs[$domainID][$lid]['roomShort']?></textarea>
                            </div>
                            <div class="section txtarea big">
                                                ...
                                                if($d_ID == 6 && $attributes[$domain_cat['categoryID']]){
                                                    ...
                                                    foreach($attributes[$domain_cat['categoryID']] as $attribute) {
                                                        ...
                                                        <div class="checkLabel checkIb attr_box" data-attrtype="<?=$attribute['attrType']?>" id='attrID<?=$attribute['attrID']?>'>
															...
															<?=($ra ? '<label class="switch small test" style="float:left"><input type="checkbox" name="vis[' . $attribute['attrID'] . ']" value="' . $attribute['attrID'] . '" ' . ($ra['shown'] ? 'checked="checked"' : '') . '><span class="slider round"></span></label>' : '')?>

                                                            <label for="ch<?=$attribute['attrID']?>"><?=$attribute['defaultName']?></label>
                                                            <div class="inputLblWrap">
                                                                <div class="label">תיאור קצר: </div>
                                                                <input type="text" name="descToAttr[<?=$attribute['attrID']?>]" value="<?=outDb($roomsAttrFull[$attribute['attrID']]['attrDesc'])?>" title="">
                                                            </div>
                                                        </div>
                                                        <?php
                                                    }
                            ...
                            <div class="baseTbl">
                                ...
                                foreach($spaces as &$space) {
                                    ...
                                    <div class="tblRow spaceRow" style="cursor:pointer" id="space<?=$space['spaceID']?>" data-order="<?=$order?>" data-spaceid="<?=$space['spaceID']?>">
                                        ...
                                        <div class="tblCell">
                                            <div class="tblCell" onclick="if(confirm('האם אתה בטוח רוצה למחוק את החדר?')){location.href='/cms/moduls/minisites/rooms/popRoom.php?roomID=<?=$roomID?>&siteID=<?=$siteID?>&spaceID=<?=$space['spaceID']?>&srdel=1&openunits=true'}"><i class="fa fa-trash-o" aria-hidden="true"></i> מחיקה</div>
                        ...
                        <div class="tablWrapper doInline">
                            ...
                            <div class="baseTbl prices">
                                ...
                                <div class="tblRow">
                                    <div class="tblCell">
                                        <div class="top">מקסימום</div>
                                        <div class="bot">לאירועים</div>
                                    </div>
                                    <div class="tblCell"><input type="number" min="0" name="maxGuestsEvents" value="<?=($roomData['maxGuestsEvents'] ?? $roomData['maxGuests'] ?? 30)?>"></div>
                                </div>
                            </div>
                        </div>
                ...
                foreach(LangList::get() as $lid => $lang){ ?>
                    <div class="domain" data-id="<?=$domainID?>">
                        <div class="language" data-id="<?=$lid?>">
                            ...
                            <div class="section txtarea big">
                                <div class="label">תיאור יותר קצר</div>
                                <textarea name="roomShort" class="textEditor"><?=$roomLangs[$domainID][$lid]['roomShort']?></textarea>
                            </div>
                        </div>
                    </div>
