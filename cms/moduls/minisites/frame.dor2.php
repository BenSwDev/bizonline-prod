...
//            'masof_no_cvv' => $data['masof_no_cvv'],
        ...
        if($domainID == 1) {
            ...
            $siteData['online_order'] = $data['online_order'];
...
//$wbSite = $siteID ? ReferenceMap::from_local($siteID, 'wubook') : null;
$wbSite = ($siteID == 389) ? ReferenceMap::from_local($siteID, 'wubook') : null;
$wbActive = true;
...
<style>
    ...
    input.roundBttn:hover {background-color: #37CAF3;}
    .pagePopInside {}
    .pagePopContInside:not(:empty) {
        position: absolute;
        top: 0;
        right: 0;
        left: 0;
    ...
    .pagePopContInside iframe {
        margin-right: auto;
        height: 80%;
        margin-left: -35px;
        width:600px;max-width:60%;
    }
</style>
<div class="pagePopInside"><div class="pagePopContInside"></div></div>
<div class="editItems">
                ...
                <?if($domainID == 1) {?><div class="checkLabel checkIb ">
                    ...
                    <div class="inputLblWrap online-wrap" style="display:none;">
                        ...
                        <select name="online_order">
                            <option value="0" <?=$siteData['online_order'] == 0?"selected":""?>>ללא</option>
                            <option value="1" <?=$siteData['online_order'] == 1?"selected":""?>>ביזפופ</option>
                            <option value="2" <?=$siteData['online_order'] == 2?"selected":""?>>ספא פלוס</option>
                        </select>
                    </div>
...
<?php
    if ($wbSite){
?>
                <div class="inptLine" id="WuBookRow">
                    <div class="inputLblWrap">
                        <div class="switchTtl">WuBook פעיל</div>
                        <label class="switch">
                            <input type="checkbox" name="wbActive" value="1" <?=($wbActive ? 'checked="checked"' : '')?> />
                            <span class="slider round"></span>
                        </label>
                    </div>
                    <div class="inputLblWrap" >
                        <div class="labelTo">מזהה ב-WuBook</div>
                        <input type="text" value="<?=$wbSite->remote_id()?>" readonly title="מזהה ב-WuBook" />
                    </div>
                    <div class="inputLblWrap" style="width:120px; min-width:100px; margin:2%">
                        <input class="roundBttn" style="width:100%; margin-top:20px" value="לשנות" type="button" onclick="_openPop('externalSitePop.php?engine=WuBook&sid=<?=$siteID?>')" />
                    </div>
                    <!-- div class="inputLblWrap" style="width:150px" >
                        <input class="roundBttn" style="background-color:red; width:100%" value="לבטל" id="exCancel" type="button" onclick="" />
                    </div>
                    <div class="inputLblWrap" style="width:120px; min-width:100px; margin:2%" >
                        <input class="roundBttn" style="background-color:green; width:100%; margin-top:20px" value="פעולות" type="button" onclick="_openPop('wubookActions.php?sid=<?=$siteID?>')" />
                    </div -->
                </div>
<?php
    }
...
?>
                <div class="inptLine" id="WuBookRow">
                    <div class="inputLblWrap" >
                        <input class="roundBttn" type="button" value="WuBook" onclick="_openPop('externalSitePop.php?engine=WuBook&sid=<?=$siteID?>')" />
                    </div>
                </div>
<?php
    }
...
<script>
    ...
    function _openPop(url){
        $(".pagePopContInside").html('<iframe id="subframe' + (++fcnt) + '" frameborder=0 src="' + url + '"></iframe><div class="tabCloser" onclick="_closeTab(\'subframe_'+fcnt+'\')">x</div>');
        $(".pagePopInside").show();
    }
    function _closeTab(){
        $(".pagePopContInside").html('');
        $(".pagePopInside").hide();
    }
</script>