...
if(intval($_GET['siteID']) == 0) {
    ...
    foreach($list as $item) {
        ...
        $toalCards = udb::single_value("SELECT sum(round(gifts_purchases.sum - gifts_purchases.actualDiscount)) as totalPaid  FROM `gifts_purchases` left join giftCards on(giftCards.giftCardID = gifts_purchases.giftCardID) where `gifts_purchases`.`paid`=1 and giftCards.siteID=".$item['siteID']);
...
gifts_purchases.giftSender,gifts_purchases.famname,gifts_purchases.giftPhoneSender,gifts_purchases.sum,gifts_purchases.actualDiscount,gifts_purchases.ordersID,
