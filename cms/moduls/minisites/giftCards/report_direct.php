...
?>
        ...
        <div class="excel" style="color:#0011ff; border-color:#0011ff" onclick="location.href='?siteID=-1&month=<?=$currMonth?>&year=<?=$currYear?>'">פירוט רכישות</div>
...
/*        $sql = "select siteID,giftCardCommission from sites ";
        ...
        $que = "SELECT sites.siteID, sites.siteName, sites.giftCardCommission, sites.bankName, sites.bankNumber, sites.bankBranch, sites.bankAccount, sites.bankAcoountOwner,
                    COUNT(DISTINCT gifts_purchases.pID) AS `totalCards`, SUM(ROUND(gifts_purchases.sum - gifts_purchases.actualDiscount)) AS `totalPaid`, SUM(gifts_purchases.voucherSum) AS `totalV`, SUM(gifts_purchases.commSum) AS `total_commissions`, AVG(gifts_purchases.commPrec) AS `avgPerc`,
...
<?php
        ...
        $siteName = ($siteID > 0) ? udb::single_value("SELECT `siteName` FROM `sites` WHERE `siteID` = " . $siteID) : 'כל העסקים';
?>
...
<table cellspacing="0" id='exp_xl2'>
    <thead>
        <tr>
            <th style="width:200px">שם העסק</th>
    ...
    <tbody>
<?php
        $where[] = ($siteID > 0) ? " gifts_purchases.siteID = " . $siteID : '1';

        $que = "SELECT gifts_purchases.*, sites.siteName, SUM(giftCardsUsage.useageSum) AS `mimushSum`, GROUP_CONCAT(giftCardsUsage.comments SEPARATOR '~~~') AS `comments`
                FROM `sites` INNER JOIN `gifts_purchases` USING(`siteID`) LEFT JOIN `giftCardsUsage` USING(`pID`)
                WHERE " . implode(" AND ", $where) . "
...
?>
        <tr>
            <td colspan="7"><?=count($list)?> שוברים</td>
