...
$item = udb::single_row("SELECT `gifts_purchases`.ordersID,`gifts_purchases`.pID,`gifts_purchases`.giftSender,`gifts_purchases`.famname,`gifts_purchases`.token,`gifts_purchases`.giftPhoneSender,
                            `gifts_purchases`.bless,`gifts_purchases`.transID,`gifts_purchases`.reciveTime,`gifts_purchases`.transDate,`gifts_purchases`.sum as `paidSum`,
...
if($item) {
    ...
    <div class="gift_container"><input type="hidden" name="pID" id="pID" value="<?=$item['pID']?>"><input type="hidden" name="giftCardID" id="giftCardID" value="<?=$item['giftCardID']?>">
        <div class="gift_inside">
            ...
            <ul>
                <li>
                    ...
                    <div class="gift-worth">
                     ...
                     <div class="gift-balance" id="moneyLeft" data-avail="<?=isset($sums[$item['pID']]['totalUsage']) ? $item['paidSum'] - $sums[$item['pID']]['totalUsage'] :  $item['paidSum']?>">
                        <div class="title">יתרת גיפט קארד</div>
                        <div class="con"><?=isset($sums[$item['pID']]['totalUsage']) ? $item['paidSum'] - $sums[$item['pID']]['totalUsage'] :  $item['paidSum']?> ש"ח</div>
                    </div>
                </li>
                <li>
                    ...
                    <div class="gift-expiry">
                        ...
                        <div class="con"><?=date("d/m/Y", strtotime(" +".$item['daysValid']." months", strtotime($item['transDate'] ? $item['transDate'] : date("Y-m-d"))))?></div>
                    </div>
                    <div class="gift-num">
            ...
            <hr /><?
            $leftOver = isset($sums[$item['pID']]['totalUsage']) ? $item['paidSum'] - $sums[$item['pID']]['totalUsage'] :  $item['paidSum'];
            if($leftOver > 0) {
                ?>
                <div class="bottom-btns">
                <div class="part" onclick="mimushPop(1)">למימוש חלקי</div>
                <?if($leftOver == $item['paidSum']) {?>
