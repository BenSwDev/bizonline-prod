...
$baseIDS=array('-1' => 'מחיר רגיל', '-2' => 'מחיר קיץ וחגים');
...
if('POST' == $_SERVER['REQUEST_METHOD']) { 
	...
	$que = "SELECT IF(`basePeriod`,-`basePeriod`,`periodID`) as `tmpID`, `periodID` FROM `sitesPeriods` WHERE  basePeriod>0 AND `siteID` = ".$siteID;
	$sql = mysql_query($que) or report_error(__FILE__,__LINE__,$que);
...
$que = "SELECT sitesMinNights.*, -sitesPeriods.basePeriod as `periodID` FROM `sitesMinNights` INNER JOIN `sitesPeriods` USING(`periodID`) WHERE `basePeriod` > 0 AND `siteID` = ".$siteID;
...
<div class="editItems">
	...
	<form method="POST">
		...
		<b class="pricesTitle"><?=$baseIDS[$p_id]?>: </b>
		...
		<tr  style="font-size:12px;">
			<td>לינה בלבד</td>
			<td><select name="type1[<?=$p_id?>]" ><option value="0" <?=$p_row['type1']=="0"?"selected":""?>>לא פעיל</option><option value="1" <?=$p_row['type1']=="1"?"selected":""?>>פעיל</option></select></td>
		...
		<tr  style="font-size:12px;">
			...
			<td>ארוחת בוקר</td>
			<td><select name="type2[<?=$p_id?>]" ><option value="0" <?=$p_row['type2']=="0"?"selected":""?>>לא פעיל</option><option value="1" <?=$p_row['type2']=="1"?"selected":""?>>פעיל</option></select></td>
		...
		<tr style="font-size:12px;">
			...
			<td>חצי פנסיון</td>
			<td><select name="type3[<?=$p_id?>]" ><option value="0" <?=$p_row['type3']=="0"?"selected":""?>>לא פעיל</option><option value="1" <?=$p_row['type3']=="1"?"selected":""?>>פעיל</option></select></td>
		...
		<tr style="font-size:12px;">
			...
			<td>פנסיון מלא</td>
			<td><select name="type4[<?=$p_id?>]" ><option value="0" <?=$p_row['type4']=="0"?"selected":""?>>לא פעיל</option><option value="1" <?=$p_row['type4']=="1"?"selected":""?>>פעיל</option></select></td>
