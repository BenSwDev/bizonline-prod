...
if ('POST' == $_SERVER['REQUEST_METHOD']) {
	...
	$cp["guest"] = intval($_POST['guest'])?"1":"0";
	$cp["pageID"] = $pageID;
	...
	if($ids){
		foreach($ids as $key=>$id){
			if($id){
				$query=Array();
				$query['showorder']=$key;
				udb::update("files", $query, "`table`='MainPages' AND ref='".$galID."' AND  id=".$id."");
			}
		}
	}
}
...
if($galID){
	...
	$que="SELECT * FROM `files` WHERE `ref`=".$gallery['GalleryID']." AND `table`='MainPages' ORDER BY showorder ";
...
<div class="editItems">
	...
	<div class="miniTabs">
		...
		?>
		<div class="tab" onclick="window.location.href='<?=$men['href']?>?pageID=<?=$pageID?>&type=<?=$pageType?>'"><p><?=$men['name']?></p></div>
		<?php } ?>
	</div>
    ...
    <form method="POST" class="manageItems" enctype="multipart/form-data">
		...
		<div class="section">
			<div class="inptLine">
				...
				<div class="chkBox">
					<input type="checkbox" value="1" <?=$gallery['ifShow']?"checked":""?><?=(!$galID?"checked":"")?> name="ifShow" id="ifShow">
		...
		<div class="section">
			<div class="inptLine">
				<div class="label">גלרית אורחים</div>
				<div class="chkBox">
					<input type="checkbox" value="1" <?=$gallery['guest']?"checked":""?> name="guest" id="guest">
...
<?php } else { ?>
	<div class="manageItems">
		<div class="addButton" style="margin-top: 20px;">
			<input type="button" class="addNew" id="addNewAcc" value="הוסף חדש" onclick="window.location.href='/cms/pages/gallery.php?pageID=<?=$pageID?>&type=<?=$pageType?>&newgal=1'" >
			<?php if($galleries){ ?>
