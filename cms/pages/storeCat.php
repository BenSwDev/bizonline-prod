...
$menusList=Array();
$menusList[1]="קטגוריות מוצרים";
$menusList[2]="קטגוריות פרוייקטים";

$que="SELECT * FROM categories WHERE menuType=".$menuType." AND menuParent=0 AND LangID=".$langID." ORDER BY menuOrder";
...
<div class="manageItems" id="manageItems">
	...
	<div class="miniTabs general">
		<?php foreach($languages as $lang){ ?>
			<div class="tab<?=$lang['LangID']==$langID?" active":""?>" onclick="window.location.href='/cms/pages/storeCat.php?langID=<?=$lang['LangID']?>&menuType=<?=$menuType?>'"><p><?=$lang['LangName']?></p></div>
		<?php } ?>
	</div>
	<div class="miniTabs general">	
		<?php foreach($menusList as $key=>$mlist){ ?>
			<div class="tab<?=$key==$menuType?" active":""?>" onclick="window.location.href='/cms/pages/storeCat.php?langID=<?=$langID?>&menuType=<?=$key?>'"><p><?=$mlist?></p></div>
		<?php } ?>
	</div>

	<div class="editItems">
		<h1><?=$menuList[$menuType]?></h1>
		<form id="manageMenus" enctype="multipart/form-data">
					...
					<?php
						if($langID!=1){
							$que="SELECT MainPages_text.MainPageID, MainPages_text.MainPageTitle FROM MainPages 
								  INNER JOIN MainPages_text ON(MainPages.MainPageID = MainPages_text.MainPageID AND MainPages_text.LangID=".$langID." AND MainPages_text.MainPageTitle!='' AND MainPages_text.ifShow=1) 
								  WHERE MainPageType=".$type;
						} else {
							$que="SELECT MainPages.MainPageID, MainPages.MainPageTitle FROM MainPages WHERE MainPageType=".$type." AND ifShow=1 AND LangID=".$langID;
						}
			...
			<div class="mainCtrl">
				...
				<div class="dd" id="sortContinaer">
					...
					<?php if($menu){ foreach($menu as $m){ 
						$que="SELECT * FROM categories WHERE menuType=".$menuType." AND menuParent=".$m['menuID']." ORDER BY menuOrder";
					...
					<li class="dd-item dd3-item" data-id="<?=$m['menuID']?>"><div class="dd-handle dd3-handle"></div><div class="dd3-content" onclick="editMenu(<?=$m['menuID']?>)"><?=outDb($m['menuTitle'])?></div>
						...
						foreach($submenu as $sub){
							$que="SELECT * FROM categories WHERE menuType=".$menuType." AND menuParent=".$sub['menuID']." ORDER BY menuOrder";
...
<script>
 saveAll();
	...
	$.ajax({
		url: 'http://'+window.location.hostname+'/cms/pages/js_save.php',
		type: 'POST',
		data: formData,
		async: false,
		cache: false,
...
$('.dd').on('change', function() {
   saveAll();
	...
	$.ajax({
		url: 'http://'+window.location.hostname+'/cms/pages/js_order.php?menuType=<?=$menuType?>',
		type: 'POST',
	...
	function removeMenus(menuID){
		$.ajax({
			url: 'http://'+window.location.hostname+'/cms/pages/js_delete.php',
			type: 'POST',
			data: {menuID:menuID},
			async: false,
			success: function (myData) {
	...
	function editMenu(menuID){
		var meID = menuID;
		$.ajax({
			url: 'http://'+window.location.hostname+'/cms/pages/editmenu.php',
			type: 'POST',
			data: {menuID:meID},
			async: false,
			success: function (myData) {
