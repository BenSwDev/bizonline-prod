...
if('POST' == $_SERVER['REQUEST_METHOD']) {
	...
	$cp["suiteID"] = $pageID;
	$cp["LangID"] = 1;
	$cp["day"] = DateTime::createFromFormat("d/m/Y", $_POST['day'])->format("Y-m-d");
	...
	if($revID){
		udb::update("reviews", $cp, "reviewID =".$revID);
	} else {
		//$cp["createDay"] = date("Y-m-d");
		$revID = udb::insert("reviews", $cp);
	}
...
if($revID){
	$que="SELECT * FROM `reviews` WHERE reviewID=".$revID." ";
			...
			*/?>
			...
			<div class="section">
				<div class="inptLine">
					...
					<input type="text" readonly value="<?=($page['day']?date("d/m/Y", strtotime($page['day'])):date("d/m/Y"))?>" name="day" class="inpt datepicker">
				</div>
			</div>
			<div class="section">
				<div class="inptLine">
