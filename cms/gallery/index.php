...
if(isset($_POST['orderResult'])){
	...
	if($ids){
		foreach($ids as $key=>$id){
			if($id){
				$query=Array();
				$query['showorder']=$key;
				udb::update("files", $query, "`table`='general' AND ref='".$galID."' AND  id=".$id."");
			}
		}
	}
}
	...
	?>
	<script>window.location.href='/cms/gallery/index.php'</script>
	<?php
	exit;
...
$que="SELECT * FROM `galleries` WHERE sID=0 AND pageID=0";
...
if($galID){
	...
	$que="SELECT * FROM `files` WHERE `ref`=".$gallery['GalleryID']." AND `table`='general' ORDER BY showorder ";
    ...
    <form method="POST" class="manageItems" enctype="multipart/form-data">
		...
		<div class="section">
			<div class="inptLine">
				...
				<input type="text" value="<?=$gallery['GalleryTitle']?>" name="GalleryTitle" class="inpt">
			</div>
		</div>
		<div class="section">
			<div class="inptLine">
		...
		foreach($images as $image){ 
		$ids.=($i!=0?",":"")."imageBox_".$image['id']; ?>
		<div id="imageBox_<?=$image['id']?>">
			<div><input type="checkbox" name="imVisible[<?=$image['id']?>]" value="1" <?=$image['ifshow']==1?"checked":""?>></div>
			<div><img src="../../gallery/<?=$image['src']?>" style="max-width:100px;max-height:100px;"></div>
			<div><input type="text" name="imTitle[<?=$image['id']?>]" value="<?=$image['title']?>" placeholder="כותרת"></div>
			<div><input type="text" name="imDesc[<?=$image['id']?>]" value="<?=$image['desc']?>" placeholder="תיאור"></div>
			<div><input type="text" name="imLink[<?=$image['id']?>]" value="<?=$image['link']?>" placeholder="קישור"></div>
			<div class="remove" onclick="removeThis('<?=$image['id']?>')"><i class="fa fa-trash-o" aria-hidden="true"></i></div>
		</div>
		<?php $i++; } } ?>
...
<?php } else { ?>
	<div class="manageItems">
		<div class="addButton" style="margin-top: 20px;">
			<input type="button" class="addNew" id="addNewAcc" value="הוסף חדש"  onclick="window.location.href='/cms/gallery/index.php?newgal=1'">
			<?php if($galleries){ ?>
		...
		<table>
			...
			<?php foreach($galleries as $row) { ?>
				<tr>
					...
					<td onclick="window.location.href='/cms/gallery/index.php?gID=<?=$row['GalleryID']?>'"><?=$row['GalleryTitle']?></td>			
					<td align="center"><?=($row['ifShow']?"<span style='color:green;'>כן</span>":"<span style='color:red;'>לא</span>")?></td>
					<td align="center" class="actb">
					<div onclick="window.location.href='/cms/gallery/index.php?gID=<?=$row['GalleryID']?>'"><i class="fa fa-pencil" aria-hidden="true"></i>&nbsp;ערוך</div><div>|</div><div onClick="if(confirm('אתה בטוח??')){location.href='?gdel=<?=$row['GalleryID']?>';}" class="delete"><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;מחק</div></td>
				</tr>
			<? } ?>
			</tbody>
		</table>
